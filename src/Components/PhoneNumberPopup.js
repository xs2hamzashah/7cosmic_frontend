...
const PhoneNumberPopup = ({ isOpen, onClose, id, sellerPhoneNumber }) => {
  ...
  // Add test mode state and test seller number
  const [testMode] = useState(true);
  const testSellerNumber = "03339406277"; // Test seller number with WhatsApp
  ...
  const validatePakistaniNumber = (number) => {
    ...
    return pakistaniMobileRegex.test(cleanNumber) || pakistaniInternationalRegex.test(cleanNumber);
  };
  ...
  const redirectToWhatsApp = (phoneNumber) => {
    const internationalNumber = formatToInternationalFormat(
      testMode ? testSellerNumber : phoneNumber
    );
    console.log("Redirecting to WhatsApp with number:", internationalNumber);
  ...
  const handleSendOtp = async () => {
    if (!validatePakistaniNumber(phoneNumber)) {
      setPhoneError("Please enter a valid Pakistani mobile number (03XXXXXXXXX or +92XXXXXXXXXX)");
      return;
    }
    ...
    if (testMode) {
      ...
      console.log("Test Mode: OTP sent to:", phoneNumber);
      setIsOtpSent(true);
      setTimer(60);
      setIsLoading(false);
      return;
  ...
  const handleVerifyOtp = async () => {
    ...
    if (testMode) {
      ...
      console.log("Test Mode: Redirecting to test seller's WhatsApp:", testSellerNumber);
      redirectToWhatsApp(testSellerNumber);
      return;
    }
  ...
  return (
    <div className="popup-phone-number-overlay">
      ...
      <div className="popup-phone-number-content">
        ...
        <div>
          <h2>Enter Your Phone Number {testMode && "(Test Mode)"}</h2>
            ...
            {isOtpSent && (
              <>
                <input
                  ...
                  placeholder="Enter OTP (any number in test mode)"
                  required
                  disabled={isLoading}
                />
